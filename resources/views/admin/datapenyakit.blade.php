...
@section('content')
    ...
    <div>
      ...
      <div class="widget-box">
        ...
        <!-- konten 1 -->
       	<form method="post" action="{{url('/createdatapenyakit')}}">
        		{{csrf_field()}}
        		<div class="form-group">
        			...
        			<input type="text" name="nama" autofocus class="form-control" value="KP-{{$kdpenyakit+1}}" disabled  autocomplete="off" required>
        			<input type="hidden" name="kd_penyakit" autofocus class="form-control" value="KP-{{$kdpenyakit+1}}"  autocomplete="off" required>
        		</div>
        		<div class="form-group">
        			<label class="form-control-label" for="formGroupExampleInput2">Nama penyakit</label>
        			<input type="text" name="nm_penyakit" class="form-control" id="formGroupExampleInput2" required>
        		</div>
        		<div class="form-group">
        ...
        <!-- konten 1 -->
                <div class="table-responsive">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$no++}}</td>
                            <td>{{$values->kd_penyakit}}</td>
                            <td>{{$values->nm_penyakit}}</td>
                            <td>
    ...
    <div id="id{{$value->id}}" class="w3-modal" style="margin-top: 20px;">
        ...
        <form method="post" action="{{url('/updatepenyakit')}}">
          <header class="w3-container w3-grey"> 
            <span onclick="document.getElementById('id{{$value->id}}').style.display='none'" 
                class="w3-button w3-display-topright">&times;</span>
                <!-- header modal -->
                &nbsp;
                <p>Perbarui data untuk nama jenis kulit : <b>{{$value->nm_penyakit}}</b></p>
            </header>
            <div class="w3-container">
                ...
                <div class="form-group">
                    ...
                    <input type="text"  disabled value="{{$value->kd_penyakit}}" autofocus class="form-control" autocomplete="off" required>
                    <input type="hidden" name="kd_penyakit" value="{{$value->kd_penyakit}}" autofocus class="form-control" autocomplete="off" required>
                </div>
                <div class="form-group">
                    <label class="form-control-label" for="formGroupExampleInput">Nm. Jenis Kulit</label>
                    <input type="text" name="nm_penyakit" value="{{$value->nm_penyakit}}"  class="form-control" autocomplete="off" required>
                </div>
                </div>
            </div>
                    ...
                    if (result.value) {
                        Swal.fire(
                            ...
                            'success'
                                ...
                                window.location = "{{url('/deletedatapenyakit')}}/{{$id->id}}";
                            });
                        }
                    })
            }
